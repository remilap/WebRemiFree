...
define('IRIVEN_FORMMANAGER_UPLOAD_RANDOM_NAMES', 0);
...
class Iriven_FormManager
{
     ...
     *
     *  The file names can also be random-generated. See the {@link Iriven_FormManager_Control::set_rule() set_rule()} method and
     *  the <b>upload</b> rule
     *
     *  @var    array
     */
     ...
     *
     *  <code>
     *      $form = new Iriven_FormManager('test-form', 'post', 'process-my-form.php', array('style' => 'border:1px solid black'));
     *  </code>
     *
     ...
     *  @return void
     */
    function Iriven_FormManager($name, $method = 'POST', $action = '', $attributes = '')
    {
        ...
        $this->form_properties = array(
            ...
            'doctype'                   =>  'html',
            'fancy_form'                =>  '#' . $name . ' input[type="checkbox"], #' . $name . ' input[type="radio"], #' . $name . ' select',
            'has_upload'                =>  false,
        ...
        if (
            ...
            trigger_error('<strong>Iriven_FormManager</strong> could not automatically determine the correct path to the "process.php"
        ...
        // include the XSS filter class - the Iriven_FormManager_Control class extends this class
        require_once dirname(__FILE__) . '/includes/XSSClean.php';

        // include the Control.php file which contains the Iriven_FormManager_Control class which is
     ...
     *
     *                          -   {@link Iriven_FormManager_Button buttons}
     *                          -   {@link Iriven_FormManager_Captcha CAPTCHAs}
     *                          -   {@link Iriven_FormManager_Checkbox checkboxes}
     *                          -   {@link Iriven_FormManager_Date date pickers}
     *                          -   {@link Iriven_FormManager_File file upload controls}
     *                          -   {@link Iriven_FormManager_Hidden hidden controls}
     *                          -   {@link Iriven_FormManager_Image image button controls}
     *                          -   {@link Iriven_FormManager_Label labels}
     *                          -   {@link Iriven_FormManager_Note notes}
     *                          -   {@link Iriven_FormManager_Password password controls}
     *                          -   {@link Iriven_FormManager_Radio radio buttons}
     *                          -   {@link Iriven_FormManager_Reset reset buttons}
     *                          -   {@link Iriven_FormManager_Select selects}
     *                          -   {@link Iriven_FormManager_Submit submit buttons}
     *                          -   {@link Iriven_FormManager_Text text box controls}
     *                          -   {@link Iriven_FormManager_Textarea textareas}
     *                          -   {@link Iriven_FormManager_Time time pickers}
     *
    ...
    function &add($type)
    {
        ...
        // for all other controls
            ...
            // the classes have the "Iriven_FormManager_" prefix
            $class_name = 'Iriven_FormManager_' . ucfirst(strtolower($type));
            ...
            if (class_exists($class_name)) {
                ...
                switch ($class_name) {
                    ...
                    case 'Iriven_FormManager_Radio':
                    case 'Iriven_FormManager_Checkbox':
    ...
    /**
     *  Appends a message to an already existing {@link Iriven_FormManager_Control::set_rule() error block}
     *
     *  <code>
     *  //  create a new form
     *  $form = new Iriven_FormManager('my_form');
     ...
     *
     *  By default, the location of these files is in the same folder as Iriven_FormManager.php and the script will automatically
     ...
     *  </code>
     *
     *  To access the JavaScript object and use the pulic methods provided by it, use $('#formname').data('Iriven_FormManager')
     *  where <i>formname</i> is the form's name <b>with any dashes turned into underscores!</b>
     *
     *  <i>Therefore, if a form's name is "my-form", the JavaScript object would be accessed like $('my_form').data('Iriven_FormManager').</i>
     *
    ...
    /**
     *  Enables or disables automatic replacement of radio buttons, check boxes and select boxes with nicer ones, using
     *  the jQuery plugin.
     *
     *  @param  mixed  $selector    A string representing a valid jQuery selector indicating elements to replace, or an
     *                              empty string to replace all replaceable elements.
     *
     *                              Setting it to FALSE will instruct the library not to "fancify" the form.
     *
     *                              -   'input[type="checkbox"]' would replace only checkboxes;
     *
     *                              -   'input[type="checkbox"], input[type="radio"]' would replace checkboxes and radio
     *                                  buttons
     *
     *                              -   'input[type="checkbox"], input[type="radio"], select' would replace checkboxes,
     *                                  radio buttons and select boxes (the same as an empty string)
     *
     *                              <b>If you alter the settings, make sure to prefix the jQuery selectors with the form's
     *                              name so that checkboxes, radio buttons and select boxes are replaced only for the current
     *                              form and not page-wide! Also, make sure you are not using single quotes in the selectors!</b>
     *
     *  By default, the library makes use of the 
     *  jQuery plugin to "fancify" the form.
     *
     *  @return void
     */
    function fancy_form($selector)
    {

        $this->form_properties['fancy_form'] = $selector;
     ...
     *                                  {@link Iriven_FormManager_Control::set_rule() set_rule()}.
     *
    ...
    function render($template = '', $return = false)
    {
        ...
        foreach ($this->controls as $key => $control) {
                ...
                foreach ($control->rules as $rule => $properties) {
                    ...
                    switch ($rule) {
                        ...
                        case 'length':
                            ...
                            // the class name also containmin/max
                            $class .= '(' . $properties[0] . ',' . $properties[1] . ')';
            ...
            if (isset($attributes['type']) && $attributes['type'] == 'text' && preg_match('/\bdate\b/i', $attributes['class'])) {
                ...
                $datepicker_javascript .= '$(\'#' . $attributes['id'] . '\').Iriven_DatePicker(';
        ...
        $output .=
            '<script type="text/javascript">function ' . $function_name . '(){if(typeof jQuery=="undefined"||typeof jQuery.fn.Iriven_FormManager=="undefined"' .
            (isset($datepicker_javascript) ? '|| jQuery.fn.Iriven_DatePicker=="undefined"' : '') .
            ($this->form_properties['fancy_form'] !== false ? '|| jQuery.fn.Iriven_TransForm=="undefined"' : '') . '){setTimeout("' . $function_name . '()",100);return}' .
            (isset($datepicker_javascript) ? $datepicker_javascript : '') .
            ($this->form_properties['fancy_form'] !== false ? '$.Iriven_TransForm(\'' . $this->form_properties['fancy_form'] . '\');' : '') .
            '$("#' . $this->form_properties['name'] . '").Iriven_FormManager(' . ($javascript_object_properties != '' ? '{' . $javascript_object_properties . '}' : '') . ')}' .
     ...
     *  {@link Iriven_FormManager_Control::set_rule() error block}.
     *
     ...
     *  comply to the rules set for these controls through the {@link Iriven_FormManager_Control::set_rule() set_rule()} method.
     *
     ...
     *  available.
     *
     *  After calling this method, if there are {@link Iriven_FormManager_File file} controls on the form, you might want to check
     ...
     *  set for the control through the {@link Iriven_FormManager_Control::set_rule() set_rule()} method.
     *
                                    ...
                                    if (preg_match($format, preg_replace('/\s/', '', $attribute['value']), $segments)) {
                                        ...
                                        if ($valid) {
                                            ...
                                            if (
                                                ...
                                                $english_months[$original_month] . ' ' . $original_day . ', ' . $original_year ==
                        ...
                        case 'filetype':
                            ...
                            if (
                                ...
                                if (!isset($this->mimes)) {
                                    ...
                                    // i'm aware that in PHP 5.2+ there is json_decode, but i want this library to
    ...
    function _convert($control, $type, $jpeg_quality = 85, $preserve_original_file = false, $overwrite = false)
    {
        ...
        if (
            ...
            if ($type != $current_file_extension) {
                ...
                if (!isset($this->Iriven_Image)) {

                    // create a new instance of the image transformation class
                    $this->Iriven_Image = new Iriven_Image();

                }

                // set the source file
                $this->Iriven_Image->source_path = $this->file_upload[$control]['path'] . $this->file_upload[$control]['file_name'];

                // set the target file
                $this->Iriven_Image->target_path = $this->file_upload[$control]['path'] . $current_file_name . '.' . $type;
                ...
                $this->Iriven_Image->jpeg_quality = $jpeg_quality;

                // if there was an error when resizing the image
                if (!$this->Iriven_Image->resize(0, 0)) {
                ...
                $this->file_upload[$control]['size'] = filesize($this->Iriven_Image->target_path);
                ...
                $imageinfo = @getimagesize($this->Iriven_Image->target_path);
                ...
                if (!$preserve_original_file) {
                    ...
                    @unlink($this->Iriven_Image->source_path);
    ...
    function _resize($control, $prefix, $width, $height, $preserve_aspect_ratio = true, $method = ZEBRA_IMAGE_BOXED, $background_color = 'FFFFFF', $enlarge_smaller_images = true, $jpeg_quality = 85)
    {
        ...
        if (
            ...
            // set the target file
            $this->Iriven_Image->target_path = $this->file_upload[$control]['path'] . trim($prefix) . $this->file_upload[$control]['file_name'];

            // set whether aspect ratio should be maintained or not
            $this->Iriven_Image->maintain_ratio = $preserve_aspect_ratio;
            ...
            $this->Iriven_Image->enlarge_smaller_images = $enlarge_smaller_images;

            // if there was an error when resizing the image
            if (!$this->Iriven_Image->resize($width, $height, $method, $background_color)) {
     ...
     *                                          "IRIVEN_FORMMANAGER_UPLOAD_RANDOM_NAMES" constant instead of "FALSE") if you choose to have
