...
//===== champs deb =====
$elem = "famille";
$reqCreate = "CREATE TABLE IF NOT EXISTS ".$table_famille." (
...
if (!$resCreate) {
?>
<P><B><FONT COLOR='RED'>Erreur de création de la base <?=$table_famille?></FONT></B></P>
...
if ($action == MODIF_FORM || $action == SUPPR_FORM) {
	...
	if (! isset($_REQUEST['Select'])) {
		...
		if (count($_REQUEST['Select']) != 1) {
			...
			$requete = "SELECT ".$all_fields_famille." FROM ".$table_famille." WHERE `id`='".$num."'";
...
if ($action == SUPPR_BASE) {
	...
	if (! isset($num)) {
		...
		$reqDelete = "DELETE FROM ".$table_famille." WHERE `id`='".$num."'";
...
//===== champs deb =====
		$reqUpdate = "UPDATE ".$table_famille." SET nom='".$nom."' WHERE `id`='".$num."'";
//===== champs fin =====
...
//===== champs deb =====
	...
	$reqInsert = "INSERT INTO ".$table_famille." (".$all_fields_famille.") VALUES ('".getNextFreeId($table_famille)."','".$nom."')";
//===== champs fin =====
...
<?php
	$nbElem = 0;
//===== champs deb =====
	$reqListe = "SELECT ".$all_fields_famille." FROM ".$table_famille." ORDER BY `nom` ASC";
	$resListe = $bd->execRequete($reqListe);
//===== champs fin =====
	while ($uneFamille = $bd->objetSuivant($resListe)) {
		$nbElem++;
		$options_lig = "";
		if ($nbElem % 2 == 0) $options_lig = ' BGCOLOR="Silver" ';
		echo "<TR" . $options_lig . ">\n";
		echo '<TD><INPUT TYPE="CHECKBOX" NAME="Select[]" VALUE="' . $uneFamille->id . '"></TD>'."\n";
//===== champs deb =====
		echo "<TD>" . $uneFamille->id . "</TD>\n";
		echo "<TD>" . $uneFamille->nom . "</TD>\n";
//===== champs fin =====
		echo "<TR>\n";
	}
	$msg = "";
